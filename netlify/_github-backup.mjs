...
export async function backupFeedbackRecord(record) {
  const rawKey = [
    record?.referenceId,
    String(record?.rating || ''),
    String(record?.submittedAtIso || Date.now()).replace(/[^0-9A-Za-z]/g, '')
  ].filter(Boolean).join('-');
  return backupRecord('feedback', record, rawKey || Date.now().toString());
}
