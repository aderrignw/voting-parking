import { json, getVotesStore, normalizeEircode, safeKey, validEmail, validEircode, clientIp, makeReferenceId } from './_utils.mjs';
...
export const handler = async (event) => {
  ...
  try {
    ...
    const record = {
      ...
      submittedAtIreland: now.toLocaleString('en-IE', { timeZone: 'Europe/Dublin', dateStyle: 'medium', timeStyle: 'short' }),
      userAgent: event.headers['user-agent'] || '',
      ip: clientIp(event)
    };
    await store.setJSON(key, record, { metadata: { eircode, vote, submittedAt: record.submittedAt } });
    return json(200, { ok: true, message: 'Vote successfully recorded.', record: { referenceId: publicReferenceId, eircode, vote, submittedAtIreland: record.submittedAtIreland } });
  } catch (error) {
    return json(500, { ok: false, message: 'Unable to submit your vote. Please try again.' });
  }
};
